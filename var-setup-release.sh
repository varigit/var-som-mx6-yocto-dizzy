...
if [ "$BACKEND" = "fb" ] || [ "$BACKEND" = "wayland" ]; then
     ...
     echo "QT5_IMAGE_INSTALL = \" tslib tslib-calibrate tslib-tests \"" >> $BUILD_DIR/conf/local.conf
fi

# set mesa preferred provider in case of FB or wayland backends
if [ "$BACKEND" = "x11" ]; then
     echo "#un comment for Chromium" >> $BUILD_DIR/conf/local.conf
     echo "#CORE_IMAGE_EXTRA_INSTALL += \"chromium\"" >> $BUILD_DIR/conf/local.conf
     echo "#LICENSE_FLAGS_WHITELIST= \"commercial\"" >> $BUILD_DIR/conf/local.conf
     echo "#un comment for snmp">> $BUILD_DIR/conf/local.conf
     echo "#VIRTUAL-RUNTIME_snmp-manager = \"net-snmp-server\"" >> $BUILD_DIR/conf/local.conf
     echo "#IMAGE_INSTALL_append = \" net-snmp-server net-snmp-libs net-snmp-mibs\"" >> $BUILD_DIR/conf/local.conf
